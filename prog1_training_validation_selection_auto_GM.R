...
#### STILL TO DO ####
# -DOUG: params1$elev.thresh <- 30 m on elev-p95 is that OK across Canada? -- let it vary by UTM zone?
...
full.df <- foreach (z = 1:length(paramsGL$zones), .combine='rbind', .packages=list.of.packages) %dopar% {   #add .verbose=TRUE for more info when debugging
  ...
  ## load biomass/inventory attributes database
  ...
  lidar.sample.polygons.centerpoints <- readOGR(dsn=wkg_dir, layer=paste(zone,"_pt_centerpt", sep='')) ## polygon centerpoints USE THESE INSTEAD OF lidar.hex.centerpoints, USED TO PRODUCE SHP "UTMXXX_cpt_poly_training_validation"
  #----------------------------------------------------------------------------------
  ...
  pts9.poly.training.validation.temp.5 <- subset(pts9.poly.training.validation.temp.4, select=-c(POLY250ID.y))  ## original version: remove column nr 15 to remove the duplicate of POLY250ID.x and .y
