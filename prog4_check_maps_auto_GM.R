...
param_file = "D:/Research/ANALYSES/NationalImputationForestAttributes/BAP_Imputation_working/wkg/AllUTMzones_params2.Rdata"
load(param_file)

param_file = "D:/Research/ANALYSES/NationalImputationForestAttributes/BAP_Imputation_working/wkg/AllUTMzones_params3.Rdata"
load(param_file)
...
params4$check.plot.vs.pix <- F
params4$check.temporal.val.traj <- T
params4$check.temporal.full.traj <- T
params4$check.map.coherence <- F
...
if (params4$check.plot.vs.pix) {
  ...
  for (targ in params3$targ.names.sh) {
    ...
    fig.name.str <- file.path(MapCheck.CAN.subdir, sprintf("PlotPixel_scatter_%s.pdf", targ), sep='')
  ...
  write.csv(arrange(df.to.compare, FCID), file = file.path(MapCheck.CAN.subdir, "plot_vs_pixel_predictions.csv", sep = ''))

}

#### TEMPORAL TRAJECTORIES ON VAL SET ---------------------------------------------------
...
if (params4$check.temporal.full.traj) {
  
  print('Full map boxplots:')
  
  for (targ in params3$targ.names.lg) {
    
    cmd <- sprintf('folder.name <- file.path("%s", "UTM_results", "time_results", "%s", fsep = .Platform$file.sep)', Landsat_dir, targ)  
    eval(parse(text=cmd))
    if (!file.exists(folder.name)){ next } ## if folder does not exist skip to next round of loop on targ
    ...
    idx.targ <- which(targ == params3$targ.names.lg)
    if (targ == "elev_p95") {
      dt.targ.col.name <- "p95"
      ylim.targ <- c(0,25)
      alpha.targ <- 0.2
    ...
    cmd <- sprintf('file.name <- file.path(folder.name, "persistant_treed_%sk.csv", fsep = .Platform$file.sep)', params4$nr.sampled.pix)  
    eval(parse(text=cmd))
    ...
    for (ez in 1:length(params4$ecozone.code)) {
      ...
      fig.name.str <- file.path(MapCheck.CAN.subdir, sprintf("NoChange_Full_%s_%s_1984_2012_%sKsamples.pdf", gsub(" ", "", params4$ecozone.names[ez]), params3$targ.names.plots[idx.targ], params4$nr.sampled.pix), sep='')
    ...
    for (chattr in params3$Ch_attr.classes ) {
      ...
      cmd <- sprintf('file.name <- file.path("%s", "UTM_results", "time_results", "%s", "%s_postchange_%sk_v2.csv", fsep = .Platform$file.sep)', Landsat_dir, targ, gsub(" ", "", chattr), params4$nr.sampled.pix)  
      eval(parse(text=cmd))
      ...
      for (ez in 1:length(params4$ecozone.code)) {
        ...
        fig.name.str <- file.path(MapCheck.CAN.subdir, sprintf("%s_Full_%s_%s_1984_2012_%sKsamples_v2.pdf", gsub(" ", "", chattr), gsub(" ", "", params4$ecozone.names[ez]), params3$targ.names.plots[idx.targ], params4$nr.sampled.pix), sep='')
...
}  ## end if on params4$check.temporal.full.traj
...
if (params4$check.map.coherence) {
  ...
  writeRaster(ht.low.cover.high, file.path(MapCheck.CAN.subdir, sprintf("height_below_%s_cover_above_%s", params4$lims.el_p95[1], params4$lims.p_1r_2m[2]), sep=''), format='GTiff', datatype="LOG1S", overwrite=T)
  writeRaster(ht.high.cover.low, file.path(MapCheck.CAN.subdir, sprintf("height_above_%s_cover_below_%s", params4$lims.el_p95[2], params4$lims.p_1r_2m[1]), sep=''), format='GTiff', datatype="LOG1S", overwrite=T)
