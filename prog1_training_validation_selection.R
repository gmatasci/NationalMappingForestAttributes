...
# -V integrate lidar.metrics.forest.attributes for NONBOREAL 
...
# paramsGL$zones <- c('UTM11N', 'UTM12N')
...
params1$elev.thresh.BOREAL <- 60   ## threshold on elev_p95 to eliminate birds and towers
params1$elev.thresh.NONBOREAL <- 120   ## threshold on elev_p95 to eliminate birds and towers
...
full.df.CAN <- data.frame(matrix(ncol = 4+length(params1$targ.names.lg), nrow = 0))   ## 4 fixed columns (POLY250ID, FCID, ECOZONE, TV) + those specified in params1$targ.names.lg
for (ls in 1:length(params1$lidar.sources)) {
  
  lidar.source <- params1$lidar.sources[ls]
  
  if (lidar.source == "BOREAL") {
    zones <- paramsGL$zones
    csv.dir.lidar.attr <- file.path(LOP_dir, 'LOP_attr', 'plot_cloud_metrics_first_returns_gt2m', fsep = .Platform$file.sep)
    csv.dir.forest.attr <- file.path(LOP_dir, 'LOP_attr', params1$csv.dir.name.forest.attr, fsep = .Platform$file.sep)
    ...
    elev.thresh <- params1$elev.thresh.BOREAL
    ...
    zones <- params1$transect.names
    csv.dir.lidar.attr <- file.path(LOP_dir,'LOP_attr_BC', 'plot_cloud_metrics_first_returns_gt2m', fsep = .Platform$file.sep)
    csv.dir.forest.attr <- file.path(LOP_dir, 'LOP_attr_BC', 'plot_inventory_attributes2', fsep = .Platform$file.sep)
    csv.file.name.lidar.attr <- "_lidarTable_uniqueID.csv"
    csv.file.name.forest.attr <- "_attributeTable_uniqueID.csv"
    elev.thresh <- params1$elev.thresh.NONBOREAL
    nr.clusters <- length(params1$transect.names)  ## for parallel just uncomment the foreach line, the preceding lines and the stopCluster(cl) line at the end
  }
  ...
  full.df <- foreach (z = 1:length(zones), .combine='rbind', .packages=list.of.packages) %dopar% {   #add .verbose=TRUE for more info when debugging
  ...
  #### READ & CHECK FILES -----------------------------------------------------
    ...
    infile <- file.path(csv.dir.lidar.attr, paste(zone, csv.file.name.lidar.attr, sep = ''),fsep = .Platform$file.sep)
    lidar.metrics.1streturns <- read.csv(infile, head=TRUE, sep=",")  ## cannot use fread because of issue with empty cases at end of line (one case too few)

    if ( !nrow(lidar.metrics.1streturns) == length(unique(lidar.metrics.1streturns$unique_id)) ) {
      stop(sprintf("Duplicate rows in file %s%s", zone, csv.file.name))
    }
    
    ## read biomass/inventory attributes database
    infile <- file.path(csv.dir.forest.attr, paste(zone, csv.file.name.forest.attr, sep = ''), fsep=.Platform$file.sep)
    lidar.metrics.forest.attributes <- fread(infile, header=TRUE, sep=",", showProgress=T)
  ...
  #### FILTER WRT elev-p95 (MAX & CV), 9PTS/POLYG AFTER MERGE WITH ATTR ---------------
    ...
    pct.plots.filtr.elevp95 <- sum(lidar.metrics.1streturns$elev_p95 > elev.thresh)/nrow(lidar.metrics.1streturns)  ## percent of plots filtered due to extreme elev_p95
    fileConn <- file(log.file, "a")  ## open connection to file and specify we want to append new lines
    ...
    close(fileConn)
    lidar.metrics.1streturns <- subset(lidar.metrics.1streturns, elev_p95 <= elev.thresh) 
  ...
  #### CREATE TRN/VAL SHPS ----------------------------------------------------------
    ...
    names(pts9.poly.training.validation.temp.5)[which(names(pts9.poly.training.validation.temp.5) == "POLY250ID.x")] <- "POLY250ID"
  ...
  #### GET TRN/VAL POLYGON VALUES ----------------------------------------------------------
    ...
    names(pts9.metric.attributes.training.validation.2)[which(names(pts9.metric.attributes.training.validation.2) == "POLY250ID.x")] <- "POLY250ID"
    
    ## Keep only targets of interest. Old line to keep a list of 100s
    ...
    pts9.metric.attributes.training.validation.3 <- pts9.metric.attributes.training.validation.2[, c("POLY250ID", "FCID", "ECOZONE", params1$targ.names.lg)]   ## subset to only POLY250ID and variables to average, also keep FCID (idx=1) and Ecozone column (idx=100)
  ...
  full.df.CAN <- rbind(full.df.CAN, full.df)
  
}

write.csv(arrange(full.df.CAN, FCID), file = file.path(base_wkg_dir, "lidar_metrics_mean_training_validation.csv", sep = ''))
