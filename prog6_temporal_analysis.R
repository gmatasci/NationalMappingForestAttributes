...
params6$subsetting <- F    ## to subset the full map csvs (for debugging in development phase)
params6$nr.pts.subset <- 2000
...
params6$methods <- c("RF", "YAI")   ## for which methods to run the analysis, can accept both c("RF", "YAI") 
params6$mapped.years <- seq(from=1984, to=2012)   ## only used for val boxplots when reading back the images
# params6$mapped.years <- seq(from=1984, to=1996)   
...
params6$mapped.ecozones <- list("Boreal Plains", "Boreal Shield West", "Taiga Shield West")
params6$ecozone.codes <- c(4, 6, 18)
...
params6$ylim.targ$absolute$elev_p95 <- c(0, 28)
...
#### NO CHANGE PIXELS -----------------------------------------------------------
          ...
          if (!params6$subsetting) {  ## if not subsetting read full file...
            ...
            if (file.exists(file.name.subs)) {  ## that if it already exists...
              ...
              yearly.predictions.full <- yearly.predictions.full[,.SD[sample(.N, params6$nr.pts.subset)], by=ecozone]
          ...
          if (targ == params6$targ.names[1] & method == params6$methods[1]) {  ## to do this plot only once per ecozone
            ...
            band.year.dt <- data.table(pixID=yearly.predictions.full$pixID, order=seq(from=1, to=nrow(yearly.predictions.full)))
            ...
            for (yr in 1:length(params6$mapped.years)) {
              ...
              colnames(band.year.dt)[ncol(band.year.dt)] <- as.character(params6$mapped.years[yr])
            ...
            title.str <- sprintf("Band 5 time-series:\n%s, perm. no change (%s pix.)", ecozone.name, params6$nr.pix.time.series)
            pdf(fig.name.str)
            theme_set(theme_gray(base_size = 14))
          ...
          fig.name.str <- file.path(FullBoxPlots.subdir, sprintf("NoChange_Full_%s_%s_%s_1984_2012.pdf", 
                                                                       gsub(" ", "", ecozone.name), 
                                                                       params6$targ.names.plots[idx.targ], 
                                                                       method), sep='')
          ...
          fig.name.str <- file.path(FullPixelTimeSeries.subdir, sprintf("NoChange_PixTimeSeries_Full_%s_%s_%s_1984_2012.pdf", 
                                                                 ...
                                                                 params6$targ.names.plots[idx.targ], 
                                                                 method), sep='')
          title.str <- sprintf("Pixel time-series:\n%s, %s, perm. no change (%s pix.)", ecozone.name, method, params6$nr.pix.time.series )
          pdf(fig.name.str)
          theme_set(theme_gray(base_size = 14))
  ...
  #### CHANGE PIXELS -----------------------------------------------------------
          ...
          #   
          #   cmd <- sprintf('file.name <- file.path("%s", "UTM_results", "time_results", "%s", "%s_postchange_%s_%sk_%s.csv", fsep = .Platform$file.sep)', Landsat_dir, params6$csv.version, gsub(" ", "", chattr), targ, params6$nr.sampled.pix, params6$csv.version)  
          #   eval(parse(text=cmd))
          ...
          #   yearly.predictions.full <- fread(file.name, header = T)
          #   
          #   dt.yearly.pred.by.eco.full <- yearly.predictions.full[ecozone==params6$ecozone.code[ez], c(dt.targ.col.name, "time_since"), with=F]  ## this format is already "melted"
          ...
          #     }
          #   }
          #   fig.name.str <- file.path(FullBoxPlots.subdir, sprintf("%s_Full_%s_%s_1984_2012_%sKsamples_%s.pdf", gsub(" ", "", chattr), gsub(" ", "", params6$ecozone.names[ez]), params6$targ.names.plots[idx.targ], params6$nr.sampled.pix), params6$csv.version, sep='')
          #   title.str <- sprintf("%s, %s", params6$ecozone.names[ez], chattr)
          #   pdf(fig.name.str)
          #     theme_set(theme_gray(base_size = 14))
          #     
          ...
          #   dev.off()
          #     
          # } ## end for on params6$Ch_attr.classes
